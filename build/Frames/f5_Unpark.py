...
from behind import button_color, go_unpark, lotcheck

from pathlib import Path
OUTPUT_PATH = Path(__file__).parent
ASSETS_PATH = OUTPUT_PATH / Path(r"C:\Users\alama\OneDrive\Desktop\Parking Lot System\build\assets\frame4")


def relative_to_assets(path: str) -> Path:
    return ASSETS_PATH / Path(path)

class UnparkPage(Frame):
    ...
    def __init__(self, master):
        ...
        parklotnum = Entry( self, bd=0, bg="#EBEBEB", fg="red", highlightthickness=0,
        ...
        def new_info(new_lot):
            if lotcheck(new_lot, "Unpark") == False:
        ...
        def confirm_transaction():
            if go_unpark(parklotnum.get(), timenow.get()) == True:
